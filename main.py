...
INPUT_BUTTON_SIZE = (10, 0)
ACTION_BUTTON_SIZE = (8, 0)
...
def create_window():
    ...
    button_from = sg.CalendarButton('Naptár', 'date_from', default_date_m_d_y=(last_month_start.month, last_month_start.day, last_month_start.year), format='%Y-%m-%d', size=INPUT_BUTTON_SIZE)

    label_to = sg.Text('Eddig a dátumig', size=17)
    input_to = sg.Input(key='date_to', default_text=last_month_end.strftime("%Y-%m-%d"))
    button_to = sg.CalendarButton('Naptár', 'date_to', default_date_m_d_y=(last_month_end.month, last_month_end.day, last_month_end.year), format='%Y-%m-%d', size=INPUT_BUTTON_SIZE)

    label_export_dest = sg.Text('Excel file helye', size=17)
    input_export_dest = sg.Input(key='excel_dest')
    button_export_dest = sg.FolderBrowse('Kiválaszt', key="export_dest", size=INPUT_BUTTON_SIZE)

    button_get = sg.Button("Lekérdez", key='query', size=ACTION_BUTTON_SIZE)
    button_exit = sg.Button("Kilép", key='exit', size=ACTION_BUTTON_SIZE)
...
def response_to_xlsx(reservations_response, filepath):
    ...
    for idx, booking in enumerate(reservations_response):
        # convert the UTC date from ISO to something more pleasant to the eye
        gmt2 = dt.timezone(dt.timedelta(hours=2))
        start_time = dt.datetime.strptime(booking['StartTime'], "%Y-%m-%dT%H:%M:%S.%fZ").astimezone(gmt2).strftime("%Y-%m-%d %H:%M")
...
def create_popup(filepath):
    ...
    button_open = sg.Button("Megnyit", key='open', size=ACTION_BUTTON_SIZE)
    button_close_popup = sg.Button("Bezár", key='close', size=ACTION_BUTTON_SIZE)
...
while True:
    ...
    if event == 'query':
        ...
        date_to = values['date_to'] + 'T23:59:59.000Z'
